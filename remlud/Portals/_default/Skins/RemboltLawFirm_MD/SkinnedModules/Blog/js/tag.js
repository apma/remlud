...
function showSuggestions(el,key){workingTags=el.value.split(settings.separator);if(el.value.substring(el.value.length-settings.separator.length)==settings.separator){currentTags=el.value.split(settings.separator);hideSuggestions();}
matches=[];var i,html='',chosenTags={},tagSelected=false;currentTag={position:currentTags.length-1,tag:''};for(i=0;i<currentTags.length&&i<workingTags.length;i++){if(!tagSelected&&currentTags[i].toLowerCase()!=workingTags[i].toLowerCase()){currentTag={position:i,tag:workingTags[i].toLowerCase()};tagSelected=true;}
...
case 32:{if(settings.separator===' '){setSelection();return true;}}}
